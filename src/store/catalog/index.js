...
class CatalogState extends StoreModule {
  ...
  addItem(id) {
    this.setState(
      {
        ...this.getState(),
        additionally: [...this.getState().additionally, id]
      }
    )
  }

  removeItem(id) {
    const list  = this.getState().additionally.filter(item => item !== id)
  ...
  selectItem(id) {
    ...
    if(this.getState().inner) {

      const selectedList = this.getState().list.map(item => {
        if (item._id === id) {
          return {
                    ...item,
